...
import { isValidEmail, isValidPhone, EMAIL_HELP_TEXT, PHONE_HELP_TEXT } from '@/lib/validation';
...
const navItems: { id: Module; label: string; icon: typeof LayoutDashboard }[] = [
  ...
  { id: 'compare', label: 'Resume Compare', icon: FileText },
...
function PhoneGate() {
  ...
  return <main className="auth-page">
    ...
    <section className="auth-form-wrap"><ThemeToggle className="auth-theme-toggle" /><div className="auth-form-box">
      ...
      <form onSubmit={submit} className="auth-form">
        <label>Phone number<input required type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="98765 43210" /></label>
        {error && <div className="form-alert error"><X size={16} />{error}</div>}
...
const FOOTER_PLATFORM_LINKS: { id: Module; label: string }[] = [
  ...
  { id: 'compare', label: 'Resume Compare' },
...
function AuthScreen() {
  ...
  async function submit(event: FormEvent) {
    event.preventDefault();
    setError(''); setNotice('');
    if (mode === 'signup' && !isValidEmail(email)) {
    ...
    <section className="auth-form-wrap"><ThemeToggle className="auth-theme-toggle" /><div className="auth-form-box">
      ...
      {forgotMode ? <>
        ...
        {mode === 'signin' && authMethod === 'otp' ? (
          ...
          {mode === 'signup' && <label>Phone number<input required type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="98765 43210" /></label>}
          {error && <div className="form-alert error"><X size={16} />{error}</div>}
...
function SkillTag({ children, green = false }: { children: React.ReactNode; green?: boolean }) { return <span className={green ? 'skill-tag green' : 'skill-tag'}>{green && <Check size={12} />}{children}</span>; }
...
function Dashboard({ go }: { go: (module: Module) => void }) {
  ...
  const skillsGained = roadmap.filter((x) => x.done).length;
  ...
  return <>
    ...
    <div className="result-grid">
      <div className="content-card score-card">
        ...
        <button className="text-btn" onClick={() => go('dashboard')}>See salary comparison <ArrowRight size={15} /></button>
      </div>
    ...
    <div className="content-card roles-card">
      <SectionTitle icon={Target} title="Roles that fit your profile" action={<span className="muted-label">Based on your detected skills</span>} />
      <div className="roles-list">
        {analysis.job_roles.slice(0, 5).map((role) => {
          ...
          return <button type="button" className="role-row role-row-btn" key={role.role} onClick={async () => { await updateProfile({ target_role: role.role }); go('roadmap'); }}>
            <div className="role-rank">{role.match}%</div>
            <div className="role-info">
              <strong>{role.role}</strong>
              <div className="role-meta"><span>{owned.length} owned</span><span>{missingForRole.length} missing</span></div>
            </div>
            <div className="match-track"><div style={{ width: `${role.match}%` }} /></div>
            <div className="role-salary"><small>₹{role.salary_min}–{role.salary_max} LPA</small><strong>₹{role.salary_avg} LPA avg.</strong></div>
            <div className="role-skill-snippets">
              {owned.slice(0, 3).map((skill) => <SkillTag key={skill}>{skill}</SkillTag>)}
              {missingForRole.length > 0 && <span className="role-skill-missing">missing {missingForRole.slice(0, 4).join(', ')}{missingForRole.length > 4 ? `, +${missingForRole.length - 4} more` : ''}</span>}
            </div>
          </button>;
        })}
      </div>
      <div className="roles-footer">
        <p>Click a role to set it as your target and open its full roadmap with exact missing skills.</p>
      </div>
    </div>
...
function CompareResult({ result, roadmap, reset, profile }: { result: { old: ReturnType<typeof analyzeResumeText>; next: ReturnType<typeof analyzeResumeText> }; roadmap: RoadmapSkill[]; reset: () => void; profile: Profile | null }) {
  const { user } = useAuth();
  ...
  return <>
    ...
    <div className="content-card comparison-details">
      ...
      <div className="comparison-columns">
        <div><h3>Skills gained <span>{gained.length}</span></h3><div className="tag-cloud">{(gained.length ? gained : ['Keep learning to unlock more']).map((x) => <SkillTag green key={x}>{x}</SkillTag>)}</div></div>
...
const MODULE_DETAILS: { id: Module; title: string; icon: typeof Target; color: string }[] = [
  ...
  { id: 'compare', title: 'Resume Comparison & New Jobs', icon: FileText, color: 'purple' },
  { id: 'dashboard', title: 'Growth Dashboard', icon: LayoutDashboard, color: 'navy' },
];
