      ...
      <CertificateVerifyModal
        ...
        onCertificateVerified={async (cert) => {
          ...
          if (user) {
            await supabase.from('roadmap_skills').upsert({
              user_id: user.id,
              skill_name: cert.skillName || cert.skill || '',
              priority: 'Must Have',
              done: true,
            }, { onConflict: 'user_id,skill_name' });
            refreshProgress();
