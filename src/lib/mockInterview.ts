  ...
  'Cybersecurity': [
    {
      ...
      conceptSynonyms: [
        ['ip', 'destination', 'source', 'uncategorized', 'beacon', 'endpoint', 'address', 'host', 'asset', 'outbound', 'netflow', 'telemetry'],
        ['auth', 'ssh', 'login', 'credential', 'password', 'failed', 'session', 'account', 'stuffing', 'brute', 'log'],
        ['isolat', 'quarantine', 'edr', 'agent', 'contain', 'subnet', 'lateral', 'disconnect', 'network', 'firewall'],
        ['hash', 'virustotal', 'threat', 'intel', 'c2', 'binary', 'process', 'exfiltrat', 'ioc', 'reputation', 'payload'],
      ],
      sampleStarAnswer: {
    ...
    },
    {
      ...
      conceptSynonyms: [
        ['false positive', 'benign', 'flagged', 'alert fatigue', 'noise', 'legitimate'],
        ['false negative', 'missed', 'breach', 'undetected', 'blind spot'],
        ['tune', 'threshold', 'whitelist', 'rule', 'correlation', 'filter', 'exception', 'window', 'logic'],
      ],
      sampleStarAnswer: {
    ...
    },
    {
      ...
      conceptSynonyms: [
        ['rfc', 'header', 'return-path', 'from', 'eml', 'raw', 'envelope', 'message-id'],
        ['spf', 'dkim', 'dmarc', 'align', 'pass', 'fail', 'softfail', 'authentication'],
        ['received', 'mta', 'hop', 'originat', 'relay', 'ip', 'server'],
        ['sandbox', 'detonat', 'url', 'phish', 'payload', 'harvest', 'quarantine', 'purge', 'link'],
      ],
      sampleStarAnswer: {
  ...
  'Frontend': [
    {
      ...
      conceptSynonyms: [
        ['profiler', 'devtools', 'render', 'bottleneck', 'waste', 'flamegraph', 're-render', 'lag'],
        ['lazy', 'code split', 'dynamic import', 'bundle', 'chunk', 'suspense'],
        ['memo', 'usememo', 'usecallback', 'computation', 'overhead', 'virtual', 'tanstack'],
        ['lcp', 'cls', 'core web vital', 'lighthouse', 'layout shift', 'image', 'font', 'load time'],
      ],
      sampleStarAnswer: {
  ...
  'Backend': [
    {
      ...
      conceptSynonyms: [
        ['redis', 'memcached', 'cache', 'caching', 'ttl', 'invalidation', 'eviction'],
        ['kafka', 'rabbitmq', 'queue', 'asynchronous', 'event', 'worker', 'pool'],
        ['scale', 'load balancer', 'nginx', 'replica', 'cluster', 'horizontal', 'partition'],
        ['rate limit', 'circuit breaker', 'idempotency', 'idempotent', 'resilient', 'latency', 'p99'],
      ],
      sampleStarAnswer: {
...
/**
 * Checks if the candidate copied or closely rephrased the reference benchmark answer.
 * Prevents cheating and ensures candidate articulates authentic experience.
 */
function checkBenchmarkPlagiarism(candidateText: string, modelAnswer: string): boolean {
  const normCandidate = candidateText.toLowerCase().replace(/[^a-z0-9\s]/g, ' ').split(/\s+/).filter(Boolean);
  const normModel = modelAnswer.toLowerCase().replace(/[^a-z0-9\s]/g, ' ').split(/\s+/).filter(Boolean);
  if (normCandidate.length < 10 || normModel.length < 10) return false;

  const candidateStr = normCandidate.join(' ');

  // 1. Sliding window phrase matching: 8 or more consecutive words matching the benchmark
  let matchCount = 0;
  for (let i = 0; i <= normModel.length - 8; i += 2) {
    const phrase = normModel.slice(i, i + 8).join(' ');
    if (candidateStr.includes(phrase)) {
      matchCount++;
    }
  }

  // 2. High vocabulary overlap with the benchmark (>80%)
  const modelSet = new Set(normModel);
  let overlap = 0;
  for (const w of normCandidate) {
    if (modelSet.has(w)) overlap++;
  }
  const overlapRatio = overlap / normCandidate.length;

  return matchCount >= 2 || (overlapRatio >= 0.80 && normCandidate.length >= 25);
}
...
export function evaluateInterviewResponse(
  ...
  // 1. Anti-Cheating: Detect Benchmark / Reference Answer Plagiarism
  if (checkBenchmarkPlagiarism(text, modelAnswer)) {
    return {
      overallScore: 0,
      ...
      strengths: [],
      improvements: [
        '🚫 Plagiarism / Direct Benchmark Copy Detected: Your response closely matches the reference benchmark answer.',
        'In enterprise technical interviews, regurgitating canned answers or sample templates results in instant disqualification (0%).',
        'Please formulate and type your own authentic response describing your real troubleshooting steps, tools, and results.',
      ],
      modelAnswer,
    };
  }

  // 2. Strict Gibberish / Random Typing Detection
  ...
  // 3. Very Short / Incomplete Answer
  if (text.length < 35) {
    return {
      overallScore: 10,
  ...
  // 4. Calibrated Technical Criteria Evaluation
  let matchedCriteria = 0;
  const totalCriteria = question.conceptSynonyms && question.conceptSynonyms.length > 0
    ? question.conceptSynonyms.length
    : question.evaluationCriteria.length;

  if (question.conceptSynonyms && question.conceptSynonyms.length > 0) {
    for (const group of question.conceptSynonyms) {
      if (group.some(token => lower.includes(token))) {
        matchedCriteria++;
      }
    }
  } else {
  ...
  const techRatio = matchedCriteria / Math.max(1, totalCriteria);
  const techScore = matchedCriteria === 0
    ? 0
    : Math.round(techRatio * 100);

  // 5. STAR Structure Evaluation
  const hasSituation = lower.includes('when') || lower.includes('during') || lower.includes('in my') || lower.includes('alert') || lower.includes('system') || lower.includes('project') || lower.includes('shift') || lower.includes('incident');
  const hasTask = lower.includes('had to') || lower.includes('my goal') || lower.includes('responsible') || lower.includes('needed to') || lower.includes('task') || lower.includes('objective') || lower.includes('job was') || lower.includes('assigned');
  const hasAction = lower.includes('i ') || lower.includes('we ') || lower.includes('configured') || lower.includes('implemented') || lower.includes('analyzed') || lower.includes('isolated') || lower.includes('checked') || lower.includes('executed') || lower.includes('tuned') || lower.includes('quarantined') || lower.includes('extracted') || lower.includes('profiled') || lower.includes('parsed') || lower.includes('refactored');
  const hasResult = lower.includes('result') || lower.includes('reduced') || lower.includes('%') || lower.includes('prevented') || lower.includes('resolved') || lower.includes('mitigated') || lower.includes('saved') || lower.includes('minutes') || lower.includes('seconds') || lower.includes('in the end') || lower.includes('zero') || lower.includes('eliminated') || lower.includes('decreased');
  ...
  // 6. Communication & Coherence Evaluation
  const wordCount = text.split(/\s+/).length;
  let commScore = 40;
  if (wordCount >= 25) commScore = 70;
  if (wordCount >= 50) commScore = 85;
  if (wordCount >= 75) commScore = 95;
  ...
  if (matchedCriteria >= Math.ceil(totalCriteria * 0.75)) strengths.push('Excellent technical depth; thoroughly addresses core protocols, triage actions, and tools.');
  else if (matchedCriteria >= 2) strengths.push('Strong technical grounding and familiarity with core tools and protocols.');
  ...
  } else if (matchedCriteria < totalCriteria) {
